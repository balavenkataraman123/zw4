...
bindTexture(loadTexture("/static/worldgentest/gltf/grass.png?rand_num="+Math.random()), 0);
...
loadAnimation("/static/worldgentest/gltf/zombie poses/walk", "/static/worldgentest/gltf/zombie poses/walk", "walk", 30,
...
for (var prop in objNames) {
    ...
    loadObj("/static/worldgentest/gltf/"+objNames[prop]+".obj", "/static/worldgentest/gltf/"+objNames[prop]+".mtl",
        cb);
}
...
for (var prop in hbNames) {
    let pr = prop;
    function cb(res) {models[pr] = res;}
    loadObjAndHitbox("/static/worldgentest/gltf/"+hbNames[prop]+".obj", "/static/worldgentest/gltf/"+hbNames[prop]+".mtl",
        cb);
}

for (var prop in oTex) {
    var img = new Image();
    img.src = ('./static/worldgentest/gltf/gui/' + oTex[prop]).slice(1);
    oTex[prop] = img;
}
